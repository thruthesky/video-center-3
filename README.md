...
* https://github.com/thruthesky/center9-ebook 에서 책을 업로드 및 다운로드를 한다.

    * 업로드 및 관리는 별도의 웹페이지에서 하고

    * 채팅 창에서는 모두 ajax 로 통신을 한다.
...
* 전자칠판에서 자신이 그림을 그리면, 먼저 캔버스에 출력하고 서버로 보내는데, 그 그림 정보를 서버로 부터 받아서 다시 그린다.

    * 그림 데이터를 전송하는 서버 단에서 본인을 제외한 나머지 사람에게만 데이터를 전송하게 한다.
...
전자 칠판의 너비를 조절하면, 그림의 높이가 맞지 않는다.

1. 백그라운드 사진을 넣어 놓고,
2. 사진 위에 그림을 그리고,
3. 전자칠판을 좁혔다 넓혔다 하면,
4. 드로잉한 라인이 어떻게 차이나는지 알 수 있다.
...
vc.cleanWhiteboardHistory 는 전자칠판 청소. 방에 아무도 없으면, 전자칠판 그림 기록을 삭제한다.
...
# too much draw - 전자칠판에 너무 많이 그리지 못하도록 한다.
